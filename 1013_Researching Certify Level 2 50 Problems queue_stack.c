...
char pop(void){
  if(sp==0){
    return ERROR;
                  ...
                  //⇒
                  //「整数インデックス」：メリット　：動作が分かりやすい。範囲外指定の危険低。デバックしやすい。
                  //                  ：デメリット：コードが少し長くなる(buffer[sp])
                  //「ﾎﾟｲﾝﾀ(char *sp)」：メリット　：コードが簡潔になる。
                  //                  ：デメリット：ﾎﾟｲﾝﾀ演算を間違えるとメモリ破壊するバグを起こしうる。
  return (Q36);    //そうじゃなければ、buffer[rp]を返すかな？⇒✖。buffer[--sp]が正解。
                    ...
                    //感覚になってしまう。⇒
                    //「スタック」：使う指標はｓｐ１つだけ。rpやwpはキュー専用
                    //　「キュー」：rpとwpを使う。
                    //なるほど、スタックは積む場所も取る場所もそんな変わらないからspと--spとだけで場所が分かるのか。
};
